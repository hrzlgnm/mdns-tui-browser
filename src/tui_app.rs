    ...
    fn new(
        ...
        let mut state = Self {
            ...
            filter_input: InputState::new(
                "Quick Filter (typing updates live, Esc to cancel, Enter to exit)",
            ),
    ...
    fn filter_service(&self, service: &ServiceEntry) -> bool {
        ...
        if !self.filter_input.text().is_empty() {
            ...
            if has_online_keyword || has_offline_keyword || has_flapping_keyword {
                ...
                let keyword_count = has_online_keyword as u8
                    + has_offline_keyword as u8
                    + has_flapping_keyword as u8;
                if keyword_count >= 2 {
                    // Two or more keywords present - all share the same logic
...
pub async fn run_tui(
    ...
    if let Some(ref mdns_ref) = mdns {
        ...
        mdns_ref
            ...
            .map_err(|e| format!("Failed to set mDNS IP check interval: {}", e))?;
    }
                                    ...
                                    if normalized.is_empty() {
                                        ...
                                        *state.status_message.lock().await = msg;
                                        ...
                                        tokio::spawn(async move {
                                            ...
                                            {
                                                Ok(_) => {
                                                    let mut state = state_arc_inner.write().await;
                                                    state.add_service_type(&normalized_inner);
                                                    if state
                                                        .user_service_types
...
mod tests {
    ...
    fn create_test_app_state() -> AppState {
        AppState::new(HashSet::new(), false, false, None)
    }

    fn create_test_app_state_with_user_service_types(
        user_service_types: HashSet<String>,
    ) -> AppState {
        AppState::new(user_service_types, false, false, None)
    }

    fn create_flapping_sessions() -> Vec<ServiceSession> {
        vec![
            ServiceSession {
                start_time: 0,
    ...
    fn create_flapping_service(
        name: &str,
        service_type: &str,
        port: u16,
        online: bool,
    ) -> ServiceEntry {
        let flapping_sessions = create_flapping_sessions();
        let mut service = create_test_service_with_sessions(
            ...
            if online { None } else { Some(300_000_001) },
        );
    ...
    fn setup_test_state_with_user_types(user_types: Vec<&str>) -> AppState {
        ...
        AppState::new(user_types_set, false, false, None)
    }
    ...
    fn test_appstate_new() {
        let state = AppState::new(HashSet::new(), false, false, None);
    ...
    fn test_appstate_new_with_user_service_types() {
        ...
        let state = create_test_app_state_with_user_service_types(user_requested_types_set.clone());
    ...
    fn test_appstate_new_with_empty_user_service_types() {
        ...
        let state = create_test_app_state_with_user_service_types(user_requested_types);
    ...
    fn test_filter_service_flapping_keyword_only() {
        ...
        let flapping_service = create_flapping_service("test", "_http._tcp.local.", 80, true);
    ...
    fn test_filter_service_flapping_with_other_terms() {
        ...
        let flapping_http_service = create_flapping_service("test", "_http._tcp.local.", 80, true);
        ...
        let flapping_ssh_service = create_flapping_service("test", "_ssh._tcp.local.", 22, true);
    ...
    fn test_filter_service_online_flapping_combined() {
        ...
        let online_flapping_service =
            create_flapping_service("test", "_http._tcp.local.", 80, true);
        assert!(state.filter_service(&online_flapping_service));

        // Create an offline flapping service - should NOT match
        let offline_flapping_service =
            create_flapping_service("test", "_http._tcp.local.", 80, false);
    ...
    async fn test_json_state_dump() {
        ...
        let mut state = AppState::new(user_types.clone(), true, false, None);
    ...
    async fn test_json_dump_file_creation() {
        ...
        let mut state = AppState::new(user_types.clone(), false, true, None);
    ...
    async fn test_load_state_from_json() {
        ...
        let mut loaded_state = AppState::new(HashSet::new(), false, false, None);
